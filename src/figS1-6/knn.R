...
get_knn_graph <- function(rd.dat,cl, ref.cells=row.names(rd.dat), k=15, knn.outlier.th=2, outlier.frac.th=0.5,mc.cores=10,method="cosine")
{
  ...
  knn.result = get_knn_batch(rd.dat, ref.rd.dat, k, method="Annoy.Cosine", transposed = F,return.distance = T,batch.size=10000, mc.cores=mc.cores)
  knn  = knn.result[[1]]
  knn.dist = knn.result[[2]]
  ...
  pred.result = scrattch.hicat::predict_knn(knn[select.cells,], row.names(rd.dat), cl)
  ...
  knn.cl.df$cl.from.label = as.character(knn.cl.df$cl.from)
  knn.cl.df$cl.to.label = as.character(knn.cl.df$cl.to)
...
plot_constellation_2 <- function(knn.cl.df,
                                 cl.center.df,
                                 out.dir = "plot_constellation",
...
if (plot.parts == TRUE) {
  ...
  poly.Edges<- purrr::map_dfr(1:imax,function(i){
    # svMisc::progress(i)
  ...
  write_csv(poly.Edges, file=file.path(out.dir,paste0(st,"poly.edges.csv")))
  print(file.path(out.dir,paste0(st,"poly.edges.csv")))
  ...
  plot.all <- ggplot()+
    geom_polygon(data=poly.Edges,
                 alpha=0.5,
                 aes(x=x, y=y, group=Group, fill = frac))+
    geom_point(data=nodes,
               # alpha=0.8,
               shape=19,
               aes(x=x,
...
#
  g2 <- gridExtra::arrangeGrob(grobs=list(dot.size.legend,
                                          edge.width.legend,
                                          cl.center.legend), layout_matrix=layout_legend)
